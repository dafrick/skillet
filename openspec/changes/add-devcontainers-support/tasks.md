...
- [ ] 2.1 Create `.devcontainer/devcontainer.json` with `"image": "mcr.microsoft.com/devcontainers/typescript-node:1-24-bookworm"`
...
- [ ] 2.4 Add `"mounts"` with: `~/.gitconfig` read-only bind-mount at `/home/node/.gitconfig`, plus commented-out examples for `~/.claude` named volume with inline explanation of the tradeoff
- [ ] 2.5 Set `"containerEnv"` with: `GH_TOKEN: "${localEnv:GH_TOKEN}"` and `NODE_OPTIONS: "--max-old-space-size=4096"`
- [ ] 2.6 Set `"postCreateCommand": "corepack enable && corepack prepare pnpm@latest --activate && pnpm install"`
...
- [ ] 3.2 Add `devcontainer-open` target: `code --folder-uri "vscode-remote://dev-container+$(shell printf '%s' $$(pwd) | xxd -p)/workspace"` (or equivalent `devcontainer open` invocation)
...
- [ ] 4.1 Create `.devcontainer/README.md` covering: prerequisites (Docker Desktop, VS Code Dev Containers extension), step-by-step "Reopen in Container" instructions, how to set `GH_TOKEN` in the host environment, how to run `claude` inside the container and what `DEVCONTAINER=true` means, the optional `~/.claude` mount for auth persistence, and a note that `make devcontainer-build/rebuild` are available for headless use
...
- [ ] 5.4 Confirm `echo $DEVCONTAINER` returns `true` inside the container
